...
RUN dotnet build /tmp/publish/nevod.negrep/Source/Negrep -c Release /p:Version=$NG_VERSION
RUN dotnet publish /tmp/publish/nevod.negrep/Source/Negrep -c Release -f netcoreapp3.1 -r win-x86 --no-build
