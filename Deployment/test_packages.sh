...
test_package() {
    ...
    sed 's/$NG_VERSION/'"$NG_VERSION"'/g' Deployment/Test/negrep-test-package-${LINUX_DISTRIBUTION}.Dockerfile > Deployment/Test/negrep-test-package-${LINUX_DISTRIBUTION}.Dockerfile.tmp

    echo "Building..."
    docker build -t $DOCKER_IMAGE_NAME -q -f Deployment/Test/negrep-test-package-${LINUX_DISTRIBUTION}.Dockerfile.tmp . > /dev/null
    echo "Docker image has built successfully."

    docker run -t --rm $DOCKER_IMAGE_NAME

    rm Deployment/Test/negrep-test-package-${LINUX_DISTRIBUTION}.Dockerfile.tmp
}
