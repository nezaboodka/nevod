...
namespace Nezaboodka.Nevod
{
    public class StreamTextSource : TextSource
    {
        ...
        public StreamTextSource(Stream textStream, int bufferSizeInChars = DefaultBufferSizeInChars)
            : this(new StreamReader(textStream), shouldCloseReader: true, bufferSizeInChars)
        {
        }

        public StreamTextSource(TextReader reader, bool shouldCloseReader = false,
        ...
        internal static StreamTextSource FromString(string text, bool withReader,
            int bufferSizeInChars = DefaultBufferSizeInChars)
        {
            byte[] textBytes = Encoding.ASCII.GetBytes(text);
            var textStream = new MemoryStream(textBytes);
            StreamTextSource result;
            if (withReader)
            {
                var reader = new StreamReader(textStream);
                result = new StreamTextSource(reader, shouldCloseReader: true, bufferSizeInChars);
            }
            else
                result = new StreamTextSource(textStream, bufferSizeInChars);
            return result;
        }
        ...
        {
            TokenReference tokenReference = fCurrentParsedText.PlainTextTokens[tokenNumber];
            Token result = new Token(TokenKindByTokenReferenceKind[(int)tokenReference.TokenKind],
                ...
                fCurrentParsedText.GetTokenText(tokenReference),
